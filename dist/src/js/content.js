...
async function analyzeWithAI(articleContent) {
  ...
  switch (settings.aiModel) {
    ...
    case 'gemini':
      apiEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=${settings.apiKey}`;
      apiKey = settings.apiKey;
      requestBody = {
        contents: [
          {
  ...
  if (!response || !response.success) {
    throw new Error('Failed to get response from AI service: ' + (response ? response.error : 'Unknown error'));
  }
